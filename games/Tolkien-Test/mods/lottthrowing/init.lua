...
local lottthrowing_shoot_blue_arrow = function(itemstack, player, pointed_thing, drawspeed)
	local name = player:get_player_name()
	if time_bow[name] == nil then
		time_bow[name] = 0
	end
		if time_bow[name] == 0 then
			local playerpos = player:getpos()
			time_bow[name] = drawspeed
			ids[name .. " img"] = player:hud_add({
				hud_elem_type = "image",
				scale = { x = 4, y = 4 },
				position = { x = 0.9, y = 0.5 },
				name = "lottthrowing_bow_elvish.png",
				text = "lottthrowing_bow_elvish.png",
				direction = 0,
			})
			local obj = minetest.add_entity({x=playerpos.x,y=playerpos.y+1.5,z=playerpos.z}, "lottthrowing:arrow_fire_blue_entity")
			obj:get_luaentity().player = player or nil
			local dir = player:get_look_dir()
...
minetest.register_tool("lottthrowing:bow_elvish", {
	description = minetest.colorize("crimson", "High Elvish Bow")..
        minetest.get_background_escape_sequence("lightgoldenrodyellow"),
	inventory_image = "lottthrowing_bow_elvish.png",
    stack_max = 1,
    groups = {forbidden=1},
    wear = 0,
...
minetest.register_craft({
	...
	recipe = {
		{'default:mese', 'lottblocks:elf_torch', 'default:mese'},
		{'lottthrowing:arrow_fire_blue', "lottthrowing:bow_wood_mallorn", 'lottthrowing:arrow_fire_blue'},
		{'default:mese', 'lottblocks:elf_torch', 'default:mese'},
	}
})
...
minetest.register_globalstep(function()
	for _,player in ipairs(minetest.get_connected_players()) do
		...
		if not privs.zoom and player:get_wielded_item():get_name()=="lottthrowing:bow_elvish" then
			privs.zoom = true
			minetest.set_player_privs(name, privs)
		elseif privs.zoom and player:get_wielded_item():get_name()~="lottthrowing:bow_elvish" then
			privs.zoom = nil
			minetest.set_player_privs(name, privs)
		end
	end
