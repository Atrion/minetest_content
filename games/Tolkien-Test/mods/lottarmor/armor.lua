...
armor.set_player_armor = function(self, player)
			...
			for k, v in pairs(elements) do
				if v == false then
					...
					if level and not def.groups["clothes"] then
						...
						if item == "lottother:nenya" and nenya_num == 1 then
	...
	local regen = 1
	local fire_dmg = 4 - dwarf_set_items
	...
	elseif narya == false and nenya == true then
		...
		-- This is currently the ONLY thing "armor_heal" does, and it must be 10 to work
	...
	elseif narya == true and nenya == false then
		...
		-- Narya acts as a full set of mithril armor, sort of...
		-- Armor is 70, shield is 25, total of 95 for a full set.
		
		-- Having ONLY Narya is SET here to 80, so it's actually LOWER than a full set
		-- Nenya adds 20 shield armor for a total of 100
	...
	-- Rings change MAXIMUM mana, +25 each.  Lothlorien clothing increases regen, rarely dropped by
	...
	if fire_dmg ~= 4 then
		...
		-- Changes all "fire" nodes to do a set amount of damage
		
		for _, row in ipairs(ARMOR_FIRE_NODES) do
			if minetest.registered_nodes[row[1]] then
				minetest.override_item(row[1], {damage_per_second = fire_dmg})
			end
		end
	...
	if self.player_hp[name] > hp then
		...
		for i = 1, 5 do
			...
			if stack:get_count() > 0 then
                ...
                if hp <= 20 and heal == 10 then
                    playereffects.apply_effect_type("regen", 20, player)
                    
                    local pos = player:getpos()
                    effect({x = pos.x, y = pos.y + 1, z = pos.z}, 20, "heart.png", 1, 6)
