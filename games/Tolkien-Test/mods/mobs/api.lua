...
function check_for_death(self, vorpal)
	...
	if vorpal == 1 then
		minetest.sound_play( "axe_swing", { object = self.object, max_hear_distance = 15, gain = .8 } )
	
	elseif self.sounds.death then
...
do_env_damage = function(self)
	...
	or self.lava_damage ~= 0 then
		...
		or self.standing_in == "lottother:blue_flame"
		or self.standing_in == "lottother:red_flame"
		or self.standing_in == "lottother:green_flame"
		or self.standing_in == "lottother:purple_flame"
	...
	elseif self.state == "attack" then
		...
		or (self.attack_type == "dogshoot" and dist <= self.reach and dogswitch(self) == 0) then
			...
			if dist > self.reach then
				...
				if is_at_cliff(self) then
					...
					-- Modified this because guards charged even when ordered to stand guard

					if self.path.stuck and self.order ~= "stand" then
						set_velocity(self, self.walk_velocity)
		...
		or (self.attack_type == "dogshoot" and dist > self.reach and dogswitch(self) == 0) then
			...
			and random(1, 100) <= 60 then
				...
				p.y = p.y + 1.5 + (self.collisionbox[2] + self.collisionbox[5])
...
local mob_punch = function(self, hitter, tflp, tool_capabilities, dir)
	...
	and not weapon:get_definition().groups.immortal
	...
	then
		vorpal = 1
		self.health = 0
		minetest.chat_send_player(name, minetest.colorize("yellow", "***Your Dwarven axe cleaves your target in twain!"))
	...
	-- exit here if dead
	if check_for_death(self, vorpal) then
		return
	end
	...
	and tflp > punch_interval then
		...
		-- Changed value to make it less easy to fight trolls
	...
	--and not invisibility[ hitter:get_player_name() ]
	...
	then
		...
		if invisibility[ hitter:get_player_name() ] then
			
			
			invisibility[ hitter:get_player_name() ] = false
			
			playereffects.cancel_effect_type("troll_polymorph", cancel_all, hitter:get_player_name())
			playereffects.cancel_effect_type("invisible", cancel_all, hitter:get_player_name())
					
			hitter:set_physics_override(1,1,nil)
					
			hitter:set_nametag_attributes({color = {a = 255, r = 255, g = 255, b = 255}})
					
			hitter:set_properties{makes_footstep_sound = true}
			...
			multiskin:update_player_visuals(hitter)
...
function mobs:register_arrow(name, def)
	...
	minetest.register_entity(name, {
		...
		on_step = def.on_step or function(self, dtime)
			...
			-- Ben messing with arrow activation timers, because mobs hitting themselves when shooting downwards
			

			if (self.hit_player or self.hit_mob)
			-- clear mob entity before arrow becomes active
			and self.timer > (15 - (self.velocity / 2)) then
