...
minetest.register_node("bones:bones", {
	...
	can_dig = function(pos, player)
		...
		-- Overriding normal function because bones are becoming a nuisance
		
		local inv = minetest.get_meta(pos):get_inventory()
		
		--return is_owner(pos, player:get_player_name()) and inv:is_empty("main")
		
		if is_owner(pos, player:get_player_name()) and inv:is_empty("main") then
			return true
		else
			return false
		end
	...
	on_timer = function(pos, elapsed)
		...
		local exp_mins = 20
		local exp_secs = exp_mins * 60
		local destroy_secs = exp_secs * 3
		
		-- Overriding game settings, bones not going away ever
		-- Now it sets graves to NO OWNER after 20 minutes, and deletes after 60
		-- Much easier on server admin :)
		...
		if time >= exp_secs then
			
			meta:set_string("infotext", meta:get_string("owner").."'s old bones")
			meta:set_string("owner", "")
		
		elseif time > destroy_secs then
			
			-- If grave is older than 60 min remove it completely!
