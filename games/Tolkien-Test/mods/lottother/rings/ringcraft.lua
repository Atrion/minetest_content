...
local function random_break(pos, chance, output, src_time)
	...
	if math.random(8000) > chance then
		
		--16 was low, high level ring craft failed too much, frustrating.  Basically removing fail chance for now.
...
local function check_nodes(pos)
	for x = -1, 1 do
		for z = -1, 1 do
			...
			if minetest.get_node(npos).name == "default:lava_source" or minetest.get_node(npos).name == "default:lava_flowing" then
				
				-- Previously had to be a lava SOURCE, which required 9 lava buckets poured beneath.  Now one will do.
...
local function furnace_node_timer(pos, elapsed)
	...
	-- This is way less time than it used to take, but wouldn't update if you were far away, so you
	-- had to stand there for that whole time.  Now it takes a few minutes to form.
		...
		if cookable then
			...
			if src_time >= time then
				...
				--if math.random(1,3) == 1 then
					...
					--default.explode(pos, 0, 10, 60)
					
					-- Was too powerful, killed anyone near it and wouldn't update cooking status if too far out
		...
		on_place = function(itemstack, placer, pointed_thing)
			if check_nodes(pointed_thing.above) == false then
				minetest.chat_send_player(placer:get_player_name(),
					"This area is not hot enough for a ring furnace.")
				return
			end
			...
			--if not areas:canInteract(pointed_thing.above, placer:get_player_name()) then
				minetest.chat_send_player(placer:get_player_name(),
					"You cannot place a ring furnace in a protected area.")
				return
			end
			--]]
