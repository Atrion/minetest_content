...
STAMINA_TICK = 800			-- time in seconds after that 1 stamina point is taken (800)
STAMINA_TICK_MIN = 4		-- stamina ticks won't reduce stamina below this level
STAMINA_HEALTH_TICK = 16	-- time in seconds after player gets healed/damaged (4)
STAMINA_MOVE_TICK = 1		-- time in seconds after the movement is checked (0.5)

STAMINA_EXHAUST_DIG = 3		-- exhaustion increased this value after digged node (3)
STAMINA_EXHAUST_PLACE = 1	-- .. after digging node
STAMINA_EXHAUST_MOVE = 1.5	-- .. if player movement detected (1.5)
STAMINA_EXHAUST_JUMP = 0	-- .. if jumping (5)
STAMINA_EXHAUST_CRAFT = 20	-- .. if player crafts (20)
STAMINA_EXHAUST_PUNCH = 20	-- .. if player punches another player (40)
STAMINA_EXHAUST_LVL = 600	-- at what exhaustion player saturation gets lowered (160)

STAMINA_HEAL = 1		-- number of HP player gets healed after STAMINA_HEALTH_TICK
STAMINA_HEAL_LVL = 4		-- lower level of saturation needed to get healed (5)
STAMINA_STARVE = 1		-- number of HP player gets damaged by stamina after STAMINA_HEALTH_TICK
STAMINA_STARVE_LVL = 2		-- level of staturation that causes starving (3)
...
--SPRINT_DRAIN = 0.35		-- how fast to drain satation while sprinting (0-1) (0.35)
...
local function stamina_update_level(player, level)
	...
	hb.change_hudbar(player, "stamina", level, STAMINA_VISUAL_MAX)
...
minetest.register_on_joinplayer(function(player)
	...
	if minetest.get_modpath("hudbars") ~= nil then
		hb.init_hudbar(player, "stamina", level, STAMINA_VISUAL_MAX)
	else
...
if minetest.get_modpath("hudbars") ~= nil and minetest.setting_getbool("enable_damage") and ( minetest.is_yes(minetest.setting_get("enable_stamina") or "1") ) then
	hb.register_hudbar("stamina", 0xFFFFFF, S("Food"), { bar = "stamina_bar.png", icon = "stamina_hud_fg.png", bgicon = "stamina_hud_bg.png" }, 0, STAMINA_VISUAL_MAX, false)
	
	

	function stamina.hud_update(playername)
		local player = minetest.get_player_by_name(playername)
		if player ~= nil then
			hb.change_hudbar(player, "stamina", stamina.get(playername), stamina.getmax(playername))
		end
	end

	function stamina.hud_remove(playername)
	end

else
	function stamina.staminastring(playername)
		return S("stamina: @1/@2", stamina.get(playername), stamina.getmax(playername))
	end
	
	function stamina.hud_add(playername)
		local player = minetest.get_player_by_name(playername)
		local id = player:hud_add({
			hud_elem_type = "text",
			position = { x = 0.5, y=1 },
			text = stamina.staminastring(playername),
			scale = { x = 0, y = 0 },
			alignment = { x = 1, y = 0},
			direction = 1,
			number = 0xFFFFFF,
		...
		stamina.playerlist[playername].hudid = id
		return id
	end
	
	function stamina.hud_update(playername)
		local player = minetest.get_player_by_name(playername)
		player:hud_change(stamina.playerlist[playername].hudid, "text", stamina.staminastring(playername))
	end
	
	function stamina.hud_remove(playername)
		local player = minetest.get_player_by_name(playername)
		player:hud_remove(stamina.playerlist[playername].hudid)
	end
