...
function playereffects.apply_effect_type(effect_type_id, duration, player, repeat_interval_time_left)
	...
	if(repeat_interval ~= nil) then
		...
		-- This is called when timed effects end!
		
		minetest.after(duration, function(effect_id)
			playereffects.cancel_effect(effect_id)
			
			-- Added for texture update upon polymorph effects ending
				...
				if playerNode["name"] ~= "air" then
					for i=1, numParticles, 1 do
						minetest.add_particle({
							...
							vertical = false,
							texture = "hyruletools_ice_effect.png",
						})
					end
				end
			end
			
			local wielded_item = players[p]:get_wielded_item():get_name()
			
			
			if wielded_item == "lottother:dwarf_ring" 
			and not playereffects.has_effect_type(name, "invisible") 
			and not playereffects.has_effect_type(name, "rat_polymorph") 
			then
				...
				if playerNode["name"] ~= "air" then
					for i=1, numParticles, 1 do
						minetest.add_particle({
							...
							collisiondetection = true,
							vertical = false,
							texture = "hyruletools_purple_effect.png",
						})
					end
				end
				
				
			elseif wielded_item == "lottother:sting" or wielded_item == "lottother:sting_blue" then
			
				-- Check area for orcs and/or monsters
				
				local player, entity_type, obj, min_player, npc_race = nil, nil, nil, nil, nil
				local objs = minetest.get_objects_inside_radius(minetest.get_player_by_name(name):getpos(), 50)
				...
				for n = 1, #objs do
					...
					if objs[n]:is_player() then
						player = objs[n]
						playername = objs[n]:get_player_name()
						
						if minetest.get_player_privs(playername).GAMEorc and not minetest.get_player_privs(playername).GAMEwizard then
							
							-- A valiant effort on my part, but wielded item textures can't be dynamically updated :(
							-- So much for Sting glowing... best I can do is make MULTI-COLORED PARTICLES!
							
							--minetest.override_item(wielded_item, {inventory_image = "lottweapons_sting_glow.png",})
				...
				-- Tried a few different ways to make Sting "glow" when mobs around
				-- Best I could do, there's no function to change wielded item textures on the fly AFAIK
				
				-- This method actually replaces the weapon with a different one, only downside
				-- is the switch weapon animation.
				
				-- Theoretically, Glamdring should do this too :D
				
				
				if orcs == true or monsters == true then
					
					players[p]:set_wielded_item("lottother:sting_blue")
					
				else
					
					players[p]:set_wielded_item("lottother:sting")
