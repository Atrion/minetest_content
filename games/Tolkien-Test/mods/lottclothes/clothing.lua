...
clothing.set_player_clothing = function(self, player)
	...
	if elf_set > 0 then
		...
		local regen = 1 + ( .2 * elf_set )
		local regen_percent = 100 * regen
		
		mana.setregen(name, regen)
		
		minetest.chat_send_player(name, minetest.colorize("Navy", "*** Your mana regeneration is increased to " .. regen_percent .. "% by your Lothlorien clothing!"))
	end
